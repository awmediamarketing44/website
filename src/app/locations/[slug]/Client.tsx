...
export default function LocationPageClient({ slug }: { slug: string }) {
  ...
  const introParas = buildIntro(loc);
  const why = whyChoose(loc);
  ...
  return (
    <>
      ...
      <main>
        <PageHeader
          tag={`Web Design Agency · ${loc.city}`}
          title="Web Designer"
          titleAccent={loc.city}
          description={headerDescription(loc)}
        />
        ...
        <section className="py-24 border-t border-card-border">
          <div className="mx-auto max-w-7xl px-6">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-16">
              {["Why", `${loc.city} businesses choose us.`].map((line, i) => (
                <motion.span
                  key={i}
        ...
        <section className="py-24 border-t border-card-border">
          <div className="mx-auto max-w-7xl px-6">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-16">
              {["What we build", `for ${loc.city} businesses.`].map((line, i) => (
                <motion.span
                  key={i}
                  className={`block ${i > 0 ? "text-pink" : ""}`}
                  initial={{ opacity: 0, x: -30 }}
        ...
        <section className="py-24 border-t border-card-border">
          <div className="mx-auto max-w-7xl px-6">
            <h2 className="text-3xl sm:text-4xl font-bold mb-12">
              {["Other locations", "we cover."].map((line, i) => (
                <motion.span
                  key={i}
                  className={`block ${i > 0 ? "text-pink" : ""}`}
                  initial={{ opacity: 0, x: -30 }}
                ...
                .map((l, i) => (
                  ...
                  >
                    <Link
                      ...
                      className="inline-flex items-center rounded-full border border-card-border bg-card px-5 py-2.5 text-sm font-medium text-muted hover:border-pink/40 hover:text-pink transition-colors duration-200"
                    >
                      Web Designer {l.city}
                    </Link>
                  </motion.div>
